import { tag } from "@turf/turf";

export const SanKamphaengTrips = [
  {
    id: "Huai-Lan-Reservoir",
    title: {
      en: "Huai Lan Reservoir",
      zh: "怀兰水库 (Huai Lan Reservoir)",
      th: "อ่างเก็บน้ำห้วยลาน"
    },
    price: {
      en: "Admission fee 20 baht/person",
      zh: "门票：每人 20 泰铢",
      th: "ค่าบำรุงสถานที่ 20 บาท/ท่าน"
    },
    hours: {
      en: "Daily 08:00 – 18:00",
      zh: "每天 08:00 – 18:00",
      th: "เปิดทุกวัน 08:00 – 18:00 น."
    },
    detail: {
      en: "A large recreation area with beautiful scenery. Visitors enjoy picnicking, camping by the reservoir, fishing, and rowing boats. The dam road is perfect for scenic drives and photography. It is also known as a beautiful sunset viewing point and plays an important role in supporting the local ecosystem.",
      zh: "这是一个风景优美的大型休闲区。游客可以在水库旁野餐、露营、钓鱼或划船。水坝道路适合驾车观景和拍照，也是欣赏日落的绝佳地点。同时，它对当地生态系统的发展具有重要作用。",
      th: "พื้นที่พักผ่อนขนาดใหญ่ที่มีทิวทัศน์งดงาม นักท่องเที่ยวนิยมมาปิกนิก กางเต็นท์ริมอ่างเก็บน้ำ ตกปลา และพายเรือ ถนนบนสันเขื่อนเหมาะสำหรับการขับรถกินลมชมวิวและถ่ายรูป ที่นี่ยังเป็นจุดชมพระอาทิตย์ตกที่สวยงามและมีบทบาทสำคัญในการสนับสนุนระบบนิเวศในท้องถิ่น"
    },
    tag: {
      th: "ธรรมชาติ",
      en: "nature",
      zh: "自然"
    },
    detail_more: {
      location: "P644+CGF, Unnamed Road, ตำบล ออนใต้ อำเภอสันกำแพง เชียงใหม่ 50130",
      lat: 18.705984194983127, 
      lng: 99.20630264732885,
      img: "https://img.wongnai.com/p/1920x0/2022/12/25/931c38e63d2e4237b52745ddf0a27dbe.jpg",
      gallery: [
        "https://img.wongnai.com/p/1920x0/2022/11/20/e9a05038c5104c308e3927744e5d6690.jpg",
        "https://chiangmaimommy.com/wp-content/uploads/2025/06/510802172_1283965596425868_8965192434635497028_n.jpg"
      ],
      video: "",
      credit: ""
    }
  },
  {
    id: "MAIIAM-Contemporary-Art-Museum",
    title: {
      en: "MAIIAM Contemporary Art Museum",
      zh: "MAIIAM 当代艺术博物馆",
      th: "พิพิธภัณฑ์ศิลปะร่วมสมัยใหม่เอี่ยม (MAIIAM)"
    },
    price: {
      en: "Adults 200฿, Students 100฿",
      zh: "成人 200 泰铢，学生 100 泰铢",
      th: "ผู้ใหญ่ 200 บาท, นักเรียน/นักศึกษา 100 บาท"
    },
    hours: {
      en: "Every day except Tuesdays, 10 a.m. – 6 p.m.",
      zh: "除周二外每天 10:00 – 18:00",
      th: "เปิดทุกวัน (ปิดวันอังคาร) 10:00 – 18:00 น."
    },
    detail: {
      en: "Chiang Mai’s newest contemporary art museum located on San Kamphaeng Road. The museum features rotating exhibitions from Thai and international artists, short films, and creative installations. Its striking mirrored architecture has become a new landmark of Chiang Mai.",
      zh: "这是清迈最新的当代艺术博物馆，位于三甘烹路。馆内展出泰国及国际艺术家的轮换展览、短片和创意装置艺术。其独特的镜面建筑设计已成为清迈的新地标。",
      th: "พิพิธภัณฑ์ศิลปะร่วมสมัยที่โดดเด่นบนถนนสายสันกำแพง นำเสนอนิทรรศการหมุนเวียนจากศิลปินไทยและต่างชาติ ภาพยนตร์สั้น และงานศิลปะจัดวาง สถาปัตยกรรมกระจกสะท้อนแสงที่สวยงามของที่นี่ได้กลายเป็นแลนด์มาร์คแห่งใหม่ของเชียงใหม่"
    },
    tag: {
      th: "สถาปัตยกรรม",
      en: "architecture",
      zh: "建筑"
    },
    detail_more: {
      location: "122, Moo 7 Tonpao อำเภอสันกำแพง เชียงใหม่ 50130",
      lat: 18.757421868711894, 
      lng: 99.09376045654409,
      img: "https://img.kapook.com/u/2016/suppaporn/chiangmai/maiiam/cnx01.jpg",
      gallery: [
        "https://media-cdn.tripadvisor.com/media/photo-s/17/e7/dc/9e/navin-rawanchaikul.jpg",
        "https://www.hashcorner.com/wp-content/uploads/2016/10/maiiam-01.jpg"
      ],
      video: "",
      credit: ""
    }
  },
  {
    id: "Dutch-Farm-Miniature-Horse-Farm",
    title: {
      en: "Dutch Farm, Miniature Horse Farm, Chiang Mai",
      zh: "荷兰农场迷你马牧场 (Dutch Farm, Chiang Mai)",
      th: "ดัตช์ฟาร์ม ฟาร์มม้าแคระ เชียงใหม่"
    },
    price: {
      en: "Admission fee may vary (Check at entrance)",
      zh: "门票价格请现场查询",
      th: "กรุณาสอบถามค่าเข้าชมที่จุดจำหน่ายตั๋วหน้าฟาร์ม"
    },
    hours: {
      en: "Open daily from 9:00 a.m. to 6:00 p.m.",
      zh: "每天 9:00 – 18:00",
      th: "เปิดทุกวัน 09:00 – 18:00 น."
    },
    detail: {
      en: "A new tourist attraction on the San Kamphaeng–Mae On road. The farm is surrounded by mountain views and decorated in Dutch style. Visitors can get close to miniature horses, sheep, and geese, making it a fun destination for families and animal lovers.",
      zh: "位于三甘烹–湄翁路上的新旅游景点。农场四周环绕群山，采用荷兰风格装饰。游客可以近距离接触迷你马、绵羊和鹅，非常适合家庭和喜爱动物的游客。",
      th: "แหล่งท่องเที่ยวสวยงามบนถนนสันกำแพง-แม่ออน ฟาร์มล้อมรอบด้วยวิวภูเขาและตกแต่งในสไตล์กังหันลมแบบเนเธอร์แลนด์ นักท่องเที่ยวสามารถใกล้ชิดกับม้าแคระ แกะ และห่าน เหมาะอย่างยิ่งสำหรับครอบครัวและผู้ที่รักสัตว์"
    },
    tag: {
      th: "ฟาร์ม",
      en: "farm",
      zh: "农场"
    },
    detail_more: {
      location: "56 หมู่ 5 ตำบล ออนใต้ อำเภอสันกำแพง เชียงใหม่ 50130",
      lat: 18.732934058190533, 
      lng: 99.19539341892028,
      img: "https://dynamic-media-cdn.tripadvisor.com/media/photo-o/16/9e/44/5f/dutch-farm.jpg?w=1200&h=-1&s=1",
      gallery: [
        "https://www.dutchfarm2u.com/upload/3117/Xk3u90dnMw.jpg",
        "https://www.dutchfarm2u.com/upload/3117/n2CpdJUFG4.jpg"
      ],
      video: "",
      credit: ""
    }
  }
];